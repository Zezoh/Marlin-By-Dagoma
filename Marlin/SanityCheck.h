...
#if X_ENDSTOP_SERVO_NR >= 0 || Y_ENDSTOP_SERVO_NR >= 0 ||                      \
    Z_ENDSTOP_SERVO_NR >= 0
 ...
 * A probe needs a pin
 */
#if (!((HAS_Z_MIN && ENABLED(Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN)) ||           \
       HAS_Z_PROBE)) &&                                                        \
 ...
 */
    ...
    (DISABLED(Z_MIN_PROBE_ENDSTOP) ||                                          \
     ENABLED(                                                                  \
         DISABLE_Z_MIN_PROBE_ENDSTOP)) // It's possible for someone to set a pin
                                       // for the Z probe, but not enable it.
...
#endif
// Forcing Servo definitions can break some hall effect sensor setups. Leaving
// these here for further comment.
// #ifndef NUM_SERVOS
//  #error You must have NUM_SERVOS defined and there must be at least 1
//  configured to use Z_MIN_PROBE_ENDSTOP.
// #endif
// #if defined(NUM_SERVOS) && NUM_SERVOS < 1
//  #error You must have at least 1 servo defined for NUM_SERVOS to use
//  Z_MIN_PROBE_ENDSTOP.
// #endif
// #if Z_ENDSTOP_SERVO_NR < 0
//  #error You must have Z_ENDSTOP_SERVO_NR set to at least 0 or above to use
//  Z_MIN_PROBE_ENDSTOP.
// #endif
// #ifndef SERVO_ENDSTOP_ANGLES
//  #error You must have SERVO_ENDSTOP_ANGLES defined for Z Extend and Retract
//  to use Z_MIN_PROBE_ENDSTOP.
// #endif
#endif
/**
 ...
 */
    ...
    !(ENABLED(AUTO_BED_LEVELING_GRID) && ENABLED(DELTA))
...
#if EXTRUDERS == 1 || !HAS_X2_ENABLE || !HAS_X2_STEP || !HAS_X2_DIR ||         \
...
#if HOTENDS > 3
#if TEMP_SENSOR_3 == 0
#error TEMP_SENSOR_3 is required with 4 or more EXTRUDERS.
 ...
 */
    ...
    !(ENABLED(Z_DUAL_ENDSTOPS) && Z2_USE_ENDSTOP >= _XMAX_ &&                  \
      Z2_USE_ENDSTOP <= _XMIN_)
#error You must enable USE_XMIN_PLUG or USE_XMAX_PLUG
#elif DISABLED(USE_YMIN_PLUG) && DISABLED(USE_YMAX_PLUG) &&                    \
    !(ENABLED(Z_DUAL_ENDSTOPS) && Z2_USE_ENDSTOP >= _YMAX_ &&                  \
      Z2_USE_ENDSTOP <= _YMIN_)
#error You must enable USE_YMIN_PLUG or USE_YMAX_PLUG
#elif DISABLED(USE_ZMIN_PLUG) && DISABLED(USE_ZMAX_PLUG) &&                    \
    !(ENABLED(Z_DUAL_ENDSTOPS) && Z2_USE_ENDSTOP >= _ZMAX_ &&                  \
      Z2_USE_ENDSTOP <= _ZMIN_)
...
#elif DISABLED(THERMAL_PROTECTION_HOTENDS) &&                                  \
    (defined(WATCH_TEMP_PERIOD) || defined(THERMAL_PROTECTION_PERIOD))
