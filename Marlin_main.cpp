...
// For M0/M1, this flag may be cleared (by M108) to exit the wait-for-user loop
  ...
  volatile bool wait_for_click; // = false;
#endif

#if ENABLED(FILAMENT_AUTOLOAD)
  volatile bool wait_for_filament; // = false;
#endif
...
uint8_t target_extruder;
  ...
  #define ADJUST_DELTA(V) NOOP
    ...
    #define NOT_YET_CALIBRATED \
      ( \
        delta_endstop_adj[A_AXIS] == 0 && \
        delta_endstop_adj[B_AXIS] == 0 && \
        delta_endstop_adj[C_AXIS] == 0 && \
...
void clean_up_after_endstop_or_probe_move() {
  ...
  static float run_z_probe() {
    ...
    #elif MULTIPLE_PROBING == 3

      // Return the average value of all probes
      const float measured_z = z_avg;
...
void home_all_axes() { gcode_G28(true); }
  ...
  #define LOOP_CAL_ACT(VAR, _4P, _OP) LOOP_CAL_PT(VAR, _OP ? _AB : __A, _4P ? _4P_STEP : _7P_STEP)
	...
	#if ENABLED(ONE_BUTTON)
	float button_probe_pt(const float & rx, const float & ry) {

	  do_blocking_move_to_z(Z_OFFSET_CLEARANCE);
	  do_blocking_move_to_xy(0, 0);
	  planner.synchronize();

	  KEEPALIVE_STATE(PAUSED_FOR_USER);
	 
	  wait_for_click = true; // encoder sw will clear this, 
	  while (wait_for_click) idle();
  ...
  static float calibration_probe(const float &nx, const float &ny, const bool stow) {
	...
	static float probe_z_shift(const float center) {
	  ...
	  endstops.enable_z_probe(false);
	   #if ENABLED(ONE_BUTTON)
		float z_shift = button_probe_pt(0, 0) - center;
	   #else
   ...
   */
  #define G33_Q_AVG_TOLERANCE 0.02f

  inline float g33_q_probe_avg(const float &rx, const float &ry, const bool fast=false) {
    ...
    do {
      ...
      if (fast) {
        if (sample_count >= 2) {
          ...
          all_points_are_good =
            ABS(z_read[0] - z_avg) < G33_Q_AVG_TOLERANCE &&
            ABS(z_read[1] - z_avg) < G33_Q_AVG_TOLERANCE;
        }
      }
        ...
        all_points_are_good =
          ABS(z_read[0] - z_avg) < G33_Q_AVG_TOLERANCE &&
          ABS(z_read[1] - z_avg) < G33_Q_AVG_TOLERANCE &&
          ABS(z_read[2] - z_avg) < G33_Q_AVG_TOLERANCE;
      }
  ...
  inline void gcode_G33() {
    ...
    if (parser.seen('Q')) {
      ...
      float tower1_altitude = g33_q_probe_avg(delta_tower[A_AXIS][X_AXIS], delta_tower[A_AXIS][Y_AXIS]);
      float tower2_altitude = g33_q_probe_avg(delta_tower[B_AXIS][X_AXIS], delta_tower[B_AXIS][Y_AXIS]);
      float tower3_altitude = g33_q_probe_avg(delta_tower[C_AXIS][X_AXIS], delta_tower[C_AXIS][Y_AXIS]);
      float center_altitude = g33_q_probe_avg(0.0f, 0.0f);
      ...
      do {
        ...
        tower1_altitude = g33_q_probe_avg(delta_tower[A_AXIS][X_AXIS], delta_tower[A_AXIS][Y_AXIS]);
        tower2_altitude = g33_q_probe_avg(delta_tower[B_AXIS][X_AXIS], delta_tower[B_AXIS][Y_AXIS]);
        tower3_altitude = g33_q_probe_avg(delta_tower[C_AXIS][X_AXIS], delta_tower[C_AXIS][Y_AXIS]);
        center_altitude = g33_q_probe_avg(0.0f, 0.0f);
    ...
    // Report settings
    const char *checkingac = PSTR("Checking...");
...
inline void gcode_M17() {
  ...
  static void wait_for_filament_reload(const int8_t max_beep_count=0) {
    ...
    KEEPALIVE_STATE(PAUSED_FOR_USER);
      ...
      if (nozzle_timed_out) {
		...
		#if ENABLED(ONE_BUTTON)
          while (wait_for_filament) idle(true);
	     #else
          while (wait_for_user) idle(true); // LCD click or M108 will clear this
	    #endif
		...
		#if ENABLED(ONE_BUTTON)
          wait_for_filament = false; // Wait for user to load filament
	     #else
  ...
  static void resume_print(const float &slow_load_length=0, const float &fast_load_length=0, const float &purge_length=ADVANCED_PAUSE_PURGE_LENGTH, const int8_t max_beep_count=0) {
    ...
    // Intelligent resuming
	  ...
	  if (printer_states.activity_state == ACTIVITY_CHANGING_FILAMENT) {
	    wait_for_click = true;
	    while (wait_for_click) idle(true);
	  }
	#endif
...
inline void gcode_M205() {
  ...
  inline void gcode_M665() {
    ...
    if (parser.seen('K')) delta_calibration_radius       = parser.value_float();
    if (parser.seen('X')) delta_tower_angle_trim[A_AXIS] = parser.value_float();
    if (parser.seen('Y')) delta_tower_angle_trim[B_AXIS] = parser.value_float();
    if (parser.seen('Z')) delta_tower_angle_trim[C_AXIS] = parser.value_float();
    if (parser.seen('A')) delta_diagonal_rod_trim[A_AXIS] = parser.value_float();
    if (parser.seen('B')) delta_diagonal_rod_trim[B_AXIS] = parser.value_float();
    if (parser.seen('C')) delta_diagonal_rod_trim[C_AXIS] = parser.value_float();
    recalc_delta_settings();
  }
  /**
...
inline void gcode_M502() {
  ...
  inline void gcode_M600() {
    ...
    if (pause_print(retract, park_point, unload_length, false)) {
...
#define DEFAULT_REFRESH_RATE 150
#define FAST_REFRESH_RATE 50
#define WARNING_DURATION 2000
#define NOT_CALIBRATED_DURATION 10000

// Global variables
static uint8_t stateBlink;
static uint32_t nextOneLedTick;
static uint16_t notifyWarningTimeout;
static uint8_t ledRefreshRate;

// Bit flags
#define FLAG_NOTIFY_WARNING 0x01
static uint8_t flags;

void set_notify_warning() {
    flags |= FLAG_NOTIFY_WARNING;
    notifyWarningTimeout = (millis() + WARNING_DURATION) / 100; // Store in deciseconds
    ledRefreshRate = DEFAULT_REFRESH_RATE;
}

void set_notify_not_calibrated() {
    flags |= FLAG_NOTIFY_WARNING;
    notifyWarningTimeout = (millis() + NOT_CALIBRATED_DURATION) / 100; // Store in deciseconds
    ledRefreshRate = FAST_REFRESH_RATE;
}

void manage_one_led() {
    uint32_t now = millis();
    if (now < nextOneLedTick) return;

    stateBlink = (stateBlink + 1) & 0x0F; // Use 4 bits, allowing values 0-15
    nextOneLedTick = now + ledRefreshRate;

    // LED state logic
    bool led_on;
    if (printer_states.activity_state == ACTIVITY_STARTUP_CALIBRATION) {
        led_on = (stateBlink == 0);
    } else if (flags & FLAG_NOTIFY_WARNING) {
        led_on = stateBlink & 0x01;
    } else if (printer_states.activity_state == ACTIVITY_PAUSED ||
               printer_states.activity_state == ACTIVITY_CHANGING_FILAMENT) {
        led_on = (stateBlink == 0 || stateBlink == 2);
    } else {
        led_on = (printer_states.activity_state == ACTIVITY_PRINTING);
    }

    led_on ? one_led_on() : one_led_off();

    // Check and clear warning flag
    if ((flags & FLAG_NOTIFY_WARNING) && (now / 100 >= notifyWarningTimeout)) {
        flags &= ~FLAG_NOTIFY_WARNING;
        ledRefreshRate = DEFAULT_REFRESH_RATE;
    }
}
...
void setup() {
  ...
  #if ENABLED(DELTA) && DISABLED(ONE_BUTTON)
    enqueue_and_echo_commands_P(PSTR("G28"));
  #endif
